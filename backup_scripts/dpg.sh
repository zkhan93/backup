#! /bin/bash
# script to create pgsql db dump of running pgsql server in a docker container either locally or remotely
# with a specific user.

shopt -s huponexit

function print_help {
  echo "Usage: $0 -s <server> -p <port> -u <user> -d <dest-dir> -c <container>"
  echo "  -s | --server )        server name (optional) (default: localhost)"
  echo "  -p | --port )          port to connect to (optional) (default: 5432)"
  echo "  -u | --user )          pgsql user (optional) (default: postgres)"
  echo "  -d | --dest-dir )      directory to copy backup file to"
  echo "  -c | --container )     postgres container name"
...
function stop_backup_remote {
    ...
    cid=$($ssh "ps -ef | grep 'pg_dumpall -c -U $user' | grep -v grep | awk '{print \$2}'")
...
filename="pg-dumpall-`date +%Y-%m-%d-%H-%M-%S`.gz"
tmp_dir="/var/tmp/pg-backup"
